...
Use layered least privilege: human RBAC, workload service accounts, short-lived tokens, scoped secrets, network controls, image provenance, and storage policies. Never use cluster-admin credentials inside a component.
In this chapter, **Pipeline service accounts** is treated as a production platform capability rather than a dashboard feature.
The practical project goal is: Design workload identities and service accounts for least privilege, token handling, image pulls, storage access, and Kubernetes API calls.
### 1.1 Outcome
- Explain Pipeline service accounts in architectural, operational, security, and troubleshooting terms.
...
```
### 3.2 Control plane
- **ServiceAccount:** determine ownership, lifecycle, availability, permissions, and evidence paths for this resource.
- **Role:** determine ownership, lifecycle, availability, permissions, and evidence paths for this resource.
- **RoleBinding:** determine ownership, lifecycle, availability, permissions, and evidence paths for this resource.
- **Secret:** determine ownership, lifecycle, availability, permissions, and evidence paths for this resource.
- **SCC:** determine ownership, lifecycle, availability, permissions, and evidence paths for this resource.
- **NetworkPolicy:** determine ownership, lifecycle, availability, permissions, and evidence paths for this resource.
- **image pull secret:** determine ownership, lifecycle, availability, permissions, and evidence paths for this resource.
- **OAuth token:** determine ownership, lifecycle, availability, permissions, and evidence paths for this resource.
### 3.3 Data plane
...
### 4.1 Project name: `20-pipeline-service-accounts-lab`
**Goal:** Design workload identities and service accounts for least privilege, token handling, image pulls, storage access, and Kubernetes API calls.
...
### 5.1 What makes **Pipeline service accounts** production grade
- Control **token overreach** through design-time validation, policy, observability, and a tested recovery procedure.
- Control **default service account misuse** through design-time validation, policy, observability, and a tested recovery procedure.
- Control **secret exposure** through design-time validation, policy, observability, and a tested recovery procedure.
- Control **wildcard RBAC** through design-time validation, policy, observability, and a tested recovery procedure.
- Control **cross-project access** through design-time validation, policy, observability, and a tested recovery procedure.
- Control **unsigned images** through design-time validation, policy, observability, and a tested recovery procedure.
...
**Context for Pipeline service accounts:** Evaluate architecture boundary as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate business objective as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.3. Data Contract
**Context for Pipeline service accounts:** Evaluate data contract as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate input validation as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate output validation as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.6. Idempotency
**Context for Pipeline service accounts:** Evaluate idempotency as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.7. Retry Safety
**Context for Pipeline service accounts:** Evaluate retry safety as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate timeout strategy as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate resource requests as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate resource limits as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.11. Cpu Sizing
**Context for Pipeline service accounts:** Evaluate CPU sizing as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.12. Memory Sizing
**Context for Pipeline service accounts:** Evaluate memory sizing as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.13. Gpu Scheduling
**Context for Pipeline service accounts:** Evaluate GPU scheduling as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.14. Node Placement
**Context for Pipeline service accounts:** Evaluate node placement as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate taints and tolerations as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.16. Topology
**Context for Pipeline service accounts:** Evaluate topology as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate service account as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.18. Rbac
**Context for Pipeline service accounts:** Evaluate RBAC as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate secret handling as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.20. Token Lifetime
**Context for Pipeline service accounts:** Evaluate token lifetime as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.21. Network Policy
**Context for Pipeline service accounts:** Evaluate network policy as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.22. Egress Control
**Context for Pipeline service accounts:** Evaluate egress control as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.23. Dns Dependency
**Context for Pipeline service accounts:** Evaluate DNS dependency as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.24. Proxy Behavior
**Context for Pipeline service accounts:** Evaluate proxy behavior as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.25. Tls Trust
**Context for Pipeline service accounts:** Evaluate TLS trust as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate certificate rotation as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate image provenance as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate image digest pinning as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.29. Sbom
**Context for Pipeline service accounts:** Evaluate SBOM as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate vulnerability scanning as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate signature verification as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.32. S3 Endpoint
**Context for Pipeline service accounts:** Evaluate S3 endpoint as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.33. Bucket Policy
**Context for Pipeline service accounts:** Evaluate bucket policy as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate object prefix design as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate artifact lineage as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate artifact retention as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate metadata database as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate database backup as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate database connection pool as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.40. Workspace Pvc
**Context for Pipeline service accounts:** Evaluate workspace PVC as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.41. Storage Class
**Context for Pipeline service accounts:** Evaluate storage class as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.42. Access Mode
**Context for Pipeline service accounts:** Evaluate access mode as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate capacity planning as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.44. Latency Budget
**Context for Pipeline service accounts:** Evaluate latency budget as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.45. Throughput
**Context for Pipeline service accounts:** Evaluate throughput as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate small-object performance as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.47. Caching
**Context for Pipeline service accounts:** Evaluate caching as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate cache invalidation as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate parameter design as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.50. Default Values
**Context for Pipeline service accounts:** Evaluate default values as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate schema evolution as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate version compatibility as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.53. Logging
**Context for Pipeline service accounts:** Evaluate logging as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.54. Metrics
**Context for Pipeline service accounts:** Evaluate metrics as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.55. Events
**Context for Pipeline service accounts:** Evaluate events as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.56. Audit Trail
**Context for Pipeline service accounts:** Evaluate audit trail as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.57. Correlation Id
**Context for Pipeline service accounts:** Evaluate correlation ID as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.58. Alerting
**Context for Pipeline service accounts:** Evaluate alerting as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.59. Slo
**Context for Pipeline service accounts:** Evaluate SLO as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.60. Error Budget
**Context for Pipeline service accounts:** Evaluate error budget as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.61. Runbook
**Context for Pipeline service accounts:** Evaluate runbook as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate incident ownership as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.63. Change Control
**Context for Pipeline service accounts:** Evaluate change control as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate release promotion as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.65. Rollback
**Context for Pipeline service accounts:** Evaluate rollback as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate disaster recovery as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.67. Multi-Tenancy
**Context for Pipeline service accounts:** Evaluate multi-tenancy as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.68. Quota
**Context for Pipeline service accounts:** Evaluate quota as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate cost allocation as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.70. Cleanup
**Context for Pipeline service accounts:** Evaluate cleanup as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.71. Legal Hold
**Context for Pipeline service accounts:** Evaluate legal hold as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.72. Testing
**Context for Pipeline service accounts:** Evaluate testing as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.73. Unit Test
**Context for Pipeline service accounts:** Evaluate unit test as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate integration test as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
**Context for Pipeline service accounts:** Evaluate failure injection as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.76. Load Test
**Context for Pipeline service accounts:** Evaluate load test as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.77. Gitops
**Context for Pipeline service accounts:** Evaluate GitOps as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.78. Ci Validation
**Context for Pipeline service accounts:** Evaluate CI validation as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.79. Policy As Code
**Context for Pipeline service accounts:** Evaluate policy as code as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.80. Documentation
**Context for Pipeline service accounts:** Evaluate documentation as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 6.81. Support Model
**Context for Pipeline service accounts:** Evaluate support model as part of the end-to-end operating contract. Keywords: pipeline, service, accounts.
...
### 14.1. How would you explain Pipeline service accounts to a platform architect?
**Expected L3 answer:** Address architecture, security, data, reliability, evidence, ownership, rollback, and version-specific validation for **Pipeline service accounts**.
...
- **Command 001:** `oc logs <pod> -n <project> --all-containers=true --previous`
...
- **Command 002:** `oc get pvc -n <project>`
- Purpose: collect evidence related to DNS dependency.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 003:** `oc describe pvc <pvc> -n <project>`
- Purpose: collect evidence related to proxy behavior.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 004:** `oc get sa,role,rolebinding -n <project>`
- Purpose: collect evidence related to TLS trust.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 005:** `oc auth can-i --as=system:serviceaccount:<project>:<sa> create pods -n <project>`
- Purpose: collect evidence related to certificate rotation.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 006:** `oc get secret -n <project>`
- Purpose: collect evidence related to image provenance.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 007:** `oc get route,svc,endpoints -n <project>`
- Purpose: collect evidence related to image digest pinning.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 008:** `oc get resourcequota,limitrange -n <project> -o yaml`
- Purpose: collect evidence related to SBOM.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 009:** `oc get nodes -o wide`
- Purpose: collect evidence related to vulnerability scanning.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 010:** `oc describe node <node>`
- Purpose: collect evidence related to signature verification.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 011:** `oc get networkpolicy -n <project> -o yaml`
- Purpose: collect evidence related to S3 endpoint.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 012:** `oc get dspa -n <project> -o yaml`
- Purpose: collect evidence related to bucket policy.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 013:** `oc get pods -n <project> -o wide`
- Purpose: collect evidence related to object prefix design.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 014:** `oc get events -n <project> --sort-by=.lastTimestamp`
- Purpose: collect evidence related to artifact lineage.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 015:** `oc describe pod <pod> -n <project>`
- Purpose: collect evidence related to artifact retention.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 016:** `oc logs <pod> -n <project> --all-containers=true`
- Purpose: collect evidence related to metadata database.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 017:** `oc logs <pod> -n <project> --all-containers=true --previous`
- Purpose: collect evidence related to database backup.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 018:** `oc get pvc -n <project>`
- Purpose: collect evidence related to database connection pool.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 019:** `oc describe pvc <pvc> -n <project>`
- Purpose: collect evidence related to workspace PVC.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 020:** `oc get sa,role,rolebinding -n <project>`
- Purpose: collect evidence related to storage class.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 021:** `oc auth can-i --as=system:serviceaccount:<project>:<sa> create pods -n <project>`
- Purpose: collect evidence related to access mode.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 022:** `oc get secret -n <project>`
- Purpose: collect evidence related to capacity planning.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 023:** `oc get route,svc,endpoints -n <project>`
- Purpose: collect evidence related to latency budget.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 024:** `oc get resourcequota,limitrange -n <project> -o yaml`
- Purpose: collect evidence related to throughput.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 025:** `oc get nodes -o wide`
- Purpose: collect evidence related to small-object performance.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 026:** `oc describe node <node>`
- Purpose: collect evidence related to caching.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 027:** `oc get networkpolicy -n <project> -o yaml`
- Purpose: collect evidence related to cache invalidation.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 028:** `oc get dspa -n <project> -o yaml`
- Purpose: collect evidence related to parameter design.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 029:** `oc get pods -n <project> -o wide`
- Purpose: collect evidence related to default values.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 030:** `oc get events -n <project> --sort-by=.lastTimestamp`
- Purpose: collect evidence related to schema evolution.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 031:** `oc describe pod <pod> -n <project>`
- Purpose: collect evidence related to version compatibility.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 032:** `oc logs <pod> -n <project> --all-containers=true`
- Purpose: collect evidence related to logging.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 033:** `oc logs <pod> -n <project> --all-containers=true --previous`
- Purpose: collect evidence related to metrics.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 034:** `oc get pvc -n <project>`
- Purpose: collect evidence related to events.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 035:** `oc describe pvc <pvc> -n <project>`
- Purpose: collect evidence related to audit trail.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 036:** `oc get sa,role,rolebinding -n <project>`
- Purpose: collect evidence related to correlation ID.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 037:** `oc auth can-i --as=system:serviceaccount:<project>:<sa> create pods -n <project>`
- Purpose: collect evidence related to alerting.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 038:** `oc get secret -n <project>`
- Purpose: collect evidence related to SLO.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 039:** `oc get route,svc,endpoints -n <project>`
- Purpose: collect evidence related to error budget.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 040:** `oc get resourcequota,limitrange -n <project> -o yaml`
- Purpose: collect evidence related to runbook.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 041:** `oc get nodes -o wide`
- Purpose: collect evidence related to incident ownership.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 042:** `oc describe node <node>`
- Purpose: collect evidence related to change control.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 043:** `oc get networkpolicy -n <project> -o yaml`
- Purpose: collect evidence related to release promotion.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 044:** `oc get dspa -n <project> -o yaml`
- Purpose: collect evidence related to rollback.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 045:** `oc get pods -n <project> -o wide`
- Purpose: collect evidence related to disaster recovery.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 046:** `oc get events -n <project> --sort-by=.lastTimestamp`
- Purpose: collect evidence related to multi-tenancy.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 047:** `oc describe pod <pod> -n <project>`
- Purpose: collect evidence related to quota.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 048:** `oc logs <pod> -n <project> --all-containers=true`
- Purpose: collect evidence related to cost allocation.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 049:** `oc logs <pod> -n <project> --all-containers=true --previous`
- Purpose: collect evidence related to cleanup.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 050:** `oc get pvc -n <project>`
- Purpose: collect evidence related to legal hold.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 051:** `oc describe pvc <pvc> -n <project>`
- Purpose: collect evidence related to testing.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 052:** `oc get sa,role,rolebinding -n <project>`
- Purpose: collect evidence related to unit test.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 053:** `oc auth can-i --as=system:serviceaccount:<project>:<sa> create pods -n <project>`
- Purpose: collect evidence related to integration test.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 054:** `oc get secret -n <project>`
- Purpose: collect evidence related to failure injection.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 055:** `oc get route,svc,endpoints -n <project>`
- Purpose: collect evidence related to load test.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 056:** `oc get resourcequota,limitrange -n <project> -o yaml`
- Purpose: collect evidence related to GitOps.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 057:** `oc get nodes -o wide`
- Purpose: collect evidence related to CI validation.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 058:** `oc describe node <node>`
- Purpose: collect evidence related to policy as code.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 059:** `oc get networkpolicy -n <project> -o yaml`
- Purpose: collect evidence related to documentation.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 060:** `oc get dspa -n <project> -o yaml`
- Purpose: collect evidence related to support model.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 061:** `oc get pods -n <project> -o wide`
- Purpose: collect evidence related to architecture boundary.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 062:** `oc get events -n <project> --sort-by=.lastTimestamp`
- Purpose: collect evidence related to business objective.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 063:** `oc describe pod <pod> -n <project>`
- Purpose: collect evidence related to data contract.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 064:** `oc logs <pod> -n <project> --all-containers=true`
- Purpose: collect evidence related to input validation.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 065:** `oc logs <pod> -n <project> --all-containers=true --previous`
- Purpose: collect evidence related to output validation.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 066:** `oc get pvc -n <project>`
- Purpose: collect evidence related to idempotency.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 067:** `oc describe pvc <pvc> -n <project>`
- Purpose: collect evidence related to retry safety.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 068:** `oc get sa,role,rolebinding -n <project>`
- Purpose: collect evidence related to timeout strategy.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 069:** `oc auth can-i --as=system:serviceaccount:<project>:<sa> create pods -n <project>`
- Purpose: collect evidence related to resource requests.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 070:** `oc get secret -n <project>`
- Purpose: collect evidence related to resource limits.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 071:** `oc get route,svc,endpoints -n <project>`
- Purpose: collect evidence related to CPU sizing.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 072:** `oc get resourcequota,limitrange -n <project> -o yaml`
- Purpose: collect evidence related to memory sizing.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 073:** `oc get nodes -o wide`
- Purpose: collect evidence related to GPU scheduling.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 074:** `oc describe node <node>`
- Purpose: collect evidence related to node placement.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 075:** `oc get networkpolicy -n <project> -o yaml`
- Purpose: collect evidence related to taints and tolerations.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 076:** `oc get dspa -n <project> -o yaml`
- Purpose: collect evidence related to topology.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 077:** `oc get pods -n <project> -o wide`
- Purpose: collect evidence related to service account.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 078:** `oc get events -n <project> --sort-by=.lastTimestamp`
- Purpose: collect evidence related to RBAC.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 079:** `oc describe pod <pod> -n <project>`
- Purpose: collect evidence related to secret handling.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 080:** `oc logs <pod> -n <project> --all-containers=true`
- Purpose: collect evidence related to token lifetime.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 081:** `oc logs <pod> -n <project> --all-containers=true --previous`
- Purpose: collect evidence related to network policy.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 082:** `oc get pvc -n <project>`
- Purpose: collect evidence related to egress control.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 083:** `oc describe pvc <pvc> -n <project>`
- Purpose: collect evidence related to DNS dependency.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 084:** `oc get sa,role,rolebinding -n <project>`
- Purpose: collect evidence related to proxy behavior.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 085:** `oc auth can-i --as=system:serviceaccount:<project>:<sa> create pods -n <project>`
- Purpose: collect evidence related to TLS trust.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 086:** `oc get secret -n <project>`
- Purpose: collect evidence related to certificate rotation.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 087:** `oc get route,svc,endpoints -n <project>`
- Purpose: collect evidence related to image provenance.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 088:** `oc get resourcequota,limitrange -n <project> -o yaml`
- Purpose: collect evidence related to image digest pinning.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 089:** `oc get nodes -o wide`
- Purpose: collect evidence related to SBOM.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 090:** `oc describe node <node>`
- Purpose: collect evidence related to vulnerability scanning.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 091:** `oc get networkpolicy -n <project> -o yaml`
- Purpose: collect evidence related to signature verification.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 092:** `oc get dspa -n <project> -o yaml`
- Purpose: collect evidence related to S3 endpoint.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 093:** `oc get pods -n <project> -o wide`
- Purpose: collect evidence related to bucket policy.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 094:** `oc get events -n <project> --sort-by=.lastTimestamp`
- Purpose: collect evidence related to object prefix design.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 095:** `oc describe pod <pod> -n <project>`
- Purpose: collect evidence related to artifact lineage.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 096:** `oc logs <pod> -n <project> --all-containers=true`
- Purpose: collect evidence related to artifact retention.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 097:** `oc logs <pod> -n <project> --all-containers=true --previous`
- Purpose: collect evidence related to metadata database.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 098:** `oc get pvc -n <project>`
- Purpose: collect evidence related to database backup.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 099:** `oc describe pvc <pvc> -n <project>`
- Purpose: collect evidence related to database connection pool.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 100:** `oc get sa,role,rolebinding -n <project>`
- Purpose: collect evidence related to workspace PVC.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 101:** `oc auth can-i --as=system:serviceaccount:<project>:<sa> create pods -n <project>`
- Purpose: collect evidence related to storage class.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 102:** `oc get secret -n <project>`
- Purpose: collect evidence related to access mode.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 103:** `oc get route,svc,endpoints -n <project>`
- Purpose: collect evidence related to capacity planning.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 104:** `oc get resourcequota,limitrange -n <project> -o yaml`
- Purpose: collect evidence related to latency budget.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 105:** `oc get nodes -o wide`
- Purpose: collect evidence related to throughput.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 106:** `oc describe node <node>`
- Purpose: collect evidence related to small-object performance.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 107:** `oc get networkpolicy -n <project> -o yaml`
- Purpose: collect evidence related to caching.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 108:** `oc get dspa -n <project> -o yaml`
- Purpose: collect evidence related to cache invalidation.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 109:** `oc get pods -n <project> -o wide`
- Purpose: collect evidence related to parameter design.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 110:** `oc get events -n <project> --sort-by=.lastTimestamp`
- Purpose: collect evidence related to default values.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 111:** `oc describe pod <pod> -n <project>`
- Purpose: collect evidence related to schema evolution.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 112:** `oc logs <pod> -n <project> --all-containers=true`
- Purpose: collect evidence related to version compatibility.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 113:** `oc logs <pod> -n <project> --all-containers=true --previous`
- Purpose: collect evidence related to logging.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 114:** `oc get pvc -n <project>`
- Purpose: collect evidence related to metrics.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 115:** `oc describe pvc <pvc> -n <project>`
- Purpose: collect evidence related to events.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 116:** `oc get sa,role,rolebinding -n <project>`
- Purpose: collect evidence related to audit trail.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 117:** `oc auth can-i --as=system:serviceaccount:<project>:<sa> create pods -n <project>`
- Purpose: collect evidence related to correlation ID.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 118:** `oc get secret -n <project>`
- Purpose: collect evidence related to alerting.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 119:** `oc get route,svc,endpoints -n <project>`
- Purpose: collect evidence related to SLO.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
- **Command 120:** `oc get resourcequota,limitrange -n <project> -o yaml`
- Purpose: collect evidence related to error budget.
- Safety: read the command, namespace, and output destination before execution.
- Record: timestamp, user, cluster, namespace, run, pipeline version, and change ticket.
## 16. Corporate operating standard
- **Standard 001:** For Pipeline service accounts, the runbook control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 002:** For Pipeline service accounts, the incident ownership control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 003:** For Pipeline service accounts, the change control control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 004:** For Pipeline service accounts, the release promotion control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 005:** For Pipeline service accounts, the rollback control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 006:** For Pipeline service accounts, the disaster recovery control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 007:** For Pipeline service accounts, the multi-tenancy control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 008:** For Pipeline service accounts, the quota control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 009:** For Pipeline service accounts, the cost allocation control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 010:** For Pipeline service accounts, the cleanup control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 011:** For Pipeline service accounts, the legal hold control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 012:** For Pipeline service accounts, the testing control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 013:** For Pipeline service accounts, the unit test control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 014:** For Pipeline service accounts, the integration test control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 015:** For Pipeline service accounts, the failure injection control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 016:** For Pipeline service accounts, the load test control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 017:** For Pipeline service accounts, the GitOps control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 018:** For Pipeline service accounts, the CI validation control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 019:** For Pipeline service accounts, the policy as code control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 020:** For Pipeline service accounts, the documentation control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 021:** For Pipeline service accounts, the support model control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 022:** For Pipeline service accounts, the architecture boundary control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 023:** For Pipeline service accounts, the business objective control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 024:** For Pipeline service accounts, the data contract control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 025:** For Pipeline service accounts, the input validation control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 026:** For Pipeline service accounts, the output validation control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 027:** For Pipeline service accounts, the idempotency control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 028:** For Pipeline service accounts, the retry safety control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 029:** For Pipeline service accounts, the timeout strategy control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 030:** For Pipeline service accounts, the resource requests control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 031:** For Pipeline service accounts, the resource limits control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 032:** For Pipeline service accounts, the CPU sizing control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 033:** For Pipeline service accounts, the memory sizing control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 034:** For Pipeline service accounts, the GPU scheduling control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 035:** For Pipeline service accounts, the node placement control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 036:** For Pipeline service accounts, the taints and tolerations control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 037:** For Pipeline service accounts, the topology control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 038:** For Pipeline service accounts, the service account control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 039:** For Pipeline service accounts, the RBAC control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 040:** For Pipeline service accounts, the secret handling control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 041:** For Pipeline service accounts, the token lifetime control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 042:** For Pipeline service accounts, the network policy control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 043:** For Pipeline service accounts, the egress control control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 044:** For Pipeline service accounts, the DNS dependency control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 045:** For Pipeline service accounts, the proxy behavior control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 046:** For Pipeline service accounts, the TLS trust control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 047:** For Pipeline service accounts, the certificate rotation control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 048:** For Pipeline service accounts, the image provenance control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 049:** For Pipeline service accounts, the image digest pinning control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 050:** For Pipeline service accounts, the SBOM control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 051:** For Pipeline service accounts, the vulnerability scanning control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 052:** For Pipeline service accounts, the signature verification control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 053:** For Pipeline service accounts, the S3 endpoint control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 054:** For Pipeline service accounts, the bucket policy control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 055:** For Pipeline service accounts, the object prefix design control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 056:** For Pipeline service accounts, the artifact lineage control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 057:** For Pipeline service accounts, the artifact retention control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 058:** For Pipeline service accounts, the metadata database control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 059:** For Pipeline service accounts, the database backup control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 060:** For Pipeline service accounts, the database connection pool control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 061:** For Pipeline service accounts, the workspace PVC control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 062:** For Pipeline service accounts, the storage class control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 063:** For Pipeline service accounts, the access mode control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 064:** For Pipeline service accounts, the capacity planning control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 065:** For Pipeline service accounts, the latency budget control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 066:** For Pipeline service accounts, the throughput control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 067:** For Pipeline service accounts, the small-object performance control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 068:** For Pipeline service accounts, the caching control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 069:** For Pipeline service accounts, the cache invalidation control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 070:** For Pipeline service accounts, the parameter design control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 071:** For Pipeline service accounts, the default values control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 072:** For Pipeline service accounts, the schema evolution control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 073:** For Pipeline service accounts, the version compatibility control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 074:** For Pipeline service accounts, the logging control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 075:** For Pipeline service accounts, the metrics control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 076:** For Pipeline service accounts, the events control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 077:** For Pipeline service accounts, the audit trail control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 078:** For Pipeline service accounts, the correlation ID control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 079:** For Pipeline service accounts, the alerting control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 080:** For Pipeline service accounts, the SLO control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 081:** For Pipeline service accounts, the error budget control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 082:** For Pipeline service accounts, the runbook control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 083:** For Pipeline service accounts, the incident ownership control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 084:** For Pipeline service accounts, the change control control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 085:** For Pipeline service accounts, the release promotion control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 086:** For Pipeline service accounts, the rollback control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 087:** For Pipeline service accounts, the disaster recovery control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 088:** For Pipeline service accounts, the multi-tenancy control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 089:** For Pipeline service accounts, the quota control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 090:** For Pipeline service accounts, the cost allocation control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 091:** For Pipeline service accounts, the cleanup control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 092:** For Pipeline service accounts, the legal hold control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 093:** For Pipeline service accounts, the testing control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 094:** For Pipeline service accounts, the unit test control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 095:** For Pipeline service accounts, the integration test control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 096:** For Pipeline service accounts, the failure injection control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 097:** For Pipeline service accounts, the load test control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 098:** For Pipeline service accounts, the GitOps control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 099:** For Pipeline service accounts, the CI validation control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 100:** For Pipeline service accounts, the policy as code control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 101:** For Pipeline service accounts, the documentation control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 102:** For Pipeline service accounts, the support model control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 103:** For Pipeline service accounts, the architecture boundary control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 104:** For Pipeline service accounts, the business objective control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 105:** For Pipeline service accounts, the data contract control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 106:** For Pipeline service accounts, the input validation control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 107:** For Pipeline service accounts, the output validation control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 108:** For Pipeline service accounts, the idempotency control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 109:** For Pipeline service accounts, the retry safety control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 110:** For Pipeline service accounts, the timeout strategy control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 111:** For Pipeline service accounts, the resource requests control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 112:** For Pipeline service accounts, the resource limits control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 113:** For Pipeline service accounts, the CPU sizing control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 114:** For Pipeline service accounts, the memory sizing control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 115:** For Pipeline service accounts, the GPU scheduling control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 116:** For Pipeline service accounts, the node placement control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 117:** For Pipeline service accounts, the taints and tolerations control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 118:** For Pipeline service accounts, the topology control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 119:** For Pipeline service accounts, the service account control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 120:** For Pipeline service accounts, the RBAC control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 121:** For Pipeline service accounts, the secret handling control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 122:** For Pipeline service accounts, the token lifetime control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 123:** For Pipeline service accounts, the network policy control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 124:** For Pipeline service accounts, the egress control control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 125:** For Pipeline service accounts, the DNS dependency control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 126:** For Pipeline service accounts, the proxy behavior control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 127:** For Pipeline service accounts, the TLS trust control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 128:** For Pipeline service accounts, the certificate rotation control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 129:** For Pipeline service accounts, the image provenance control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 130:** For Pipeline service accounts, the image digest pinning control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 131:** For Pipeline service accounts, the SBOM control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 132:** For Pipeline service accounts, the vulnerability scanning control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 133:** For Pipeline service accounts, the signature verification control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 134:** For Pipeline service accounts, the S3 endpoint control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 135:** For Pipeline service accounts, the bucket policy control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 136:** For Pipeline service accounts, the object prefix design control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 137:** For Pipeline service accounts, the artifact lineage control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 138:** For Pipeline service accounts, the artifact retention control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 139:** For Pipeline service accounts, the metadata database control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 140:** For Pipeline service accounts, the database backup control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 141:** For Pipeline service accounts, the database connection pool control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 142:** For Pipeline service accounts, the workspace PVC control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 143:** For Pipeline service accounts, the storage class control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 144:** For Pipeline service accounts, the access mode control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 145:** For Pipeline service accounts, the capacity planning control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 146:** For Pipeline service accounts, the latency budget control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 147:** For Pipeline service accounts, the throughput control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 148:** For Pipeline service accounts, the small-object performance control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 149:** For Pipeline service accounts, the caching control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 150:** For Pipeline service accounts, the cache invalidation control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 151:** For Pipeline service accounts, the parameter design control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 152:** For Pipeline service accounts, the default values control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 153:** For Pipeline service accounts, the schema evolution control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 154:** For Pipeline service accounts, the version compatibility control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 155:** For Pipeline service accounts, the logging control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 156:** For Pipeline service accounts, the metrics control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 157:** For Pipeline service accounts, the events control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 158:** For Pipeline service accounts, the audit trail control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 159:** For Pipeline service accounts, the correlation ID control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 160:** For Pipeline service accounts, the alerting control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 161:** For Pipeline service accounts, the SLO control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 162:** For Pipeline service accounts, the error budget control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 163:** For Pipeline service accounts, the runbook control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 164:** For Pipeline service accounts, the incident ownership control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 165:** For Pipeline service accounts, the change control control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 166:** For Pipeline service accounts, the release promotion control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 167:** For Pipeline service accounts, the rollback control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 168:** For Pipeline service accounts, the disaster recovery control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 169:** For Pipeline service accounts, the multi-tenancy control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 170:** For Pipeline service accounts, the quota control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 171:** For Pipeline service accounts, the cost allocation control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 172:** For Pipeline service accounts, the cleanup control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 173:** For Pipeline service accounts, the legal hold control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 174:** For Pipeline service accounts, the testing control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 175:** For Pipeline service accounts, the unit test control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 176:** For Pipeline service accounts, the integration test control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 177:** For Pipeline service accounts, the failure injection control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 178:** For Pipeline service accounts, the load test control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 179:** For Pipeline service accounts, the GitOps control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 180:** For Pipeline service accounts, the CI validation control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 181:** For Pipeline service accounts, the policy as code control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 182:** For Pipeline service accounts, the documentation control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 183:** For Pipeline service accounts, the support model control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 184:** For Pipeline service accounts, the architecture boundary control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 185:** For Pipeline service accounts, the business objective control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 186:** For Pipeline service accounts, the data contract control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 187:** For Pipeline service accounts, the input validation control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 188:** For Pipeline service accounts, the output validation control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 189:** For Pipeline service accounts, the idempotency control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 190:** For Pipeline service accounts, the retry safety control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 191:** For Pipeline service accounts, the timeout strategy control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 192:** For Pipeline service accounts, the resource requests control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 193:** For Pipeline service accounts, the resource limits control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 194:** For Pipeline service accounts, the CPU sizing control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
- **Standard 195:** For Pipeline service accounts, the memory sizing control must explicitly reduce the risk of wildcard RBAC; evidence must be reviewable by another engineer.
- **Standard 196:** For Pipeline service accounts, the GPU scheduling control must explicitly reduce the risk of cross-project access; evidence must be reviewable by another engineer.
- **Standard 197:** For Pipeline service accounts, the node placement control must explicitly reduce the risk of unsigned images; evidence must be reviewable by another engineer.
- **Standard 198:** For Pipeline service accounts, the taints and tolerations control must explicitly reduce the risk of token overreach; evidence must be reviewable by another engineer.
- **Standard 199:** For Pipeline service accounts, the topology control must explicitly reduce the risk of default service account misuse; evidence must be reviewable by another engineer.
- **Standard 200:** For Pipeline service accounts, the service account control must explicitly reduce the risk of secret exposure; evidence must be reviewable by another engineer.
...
### 20.1. Production verification record — incident ownership
- **Topic:** Pipeline service accounts
- **Risk addressed:** wildcard RBAC
...
### 20.2. Production verification record — change control
- **Topic:** Pipeline service accounts
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.3. Production verification record — release promotion
- **Topic:** Pipeline service accounts
- **Risk addressed:** unsigned images
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.4. Production verification record — rollback
- **Topic:** Pipeline service accounts
- **Risk addressed:** token overreach
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.5. Production verification record — disaster recovery
- **Topic:** Pipeline service accounts
- **Risk addressed:** default service account misuse
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.6. Production verification record — multi-tenancy
- **Topic:** Pipeline service accounts
- **Risk addressed:** secret exposure
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.7. Production verification record — quota
- **Topic:** Pipeline service accounts
- **Risk addressed:** wildcard RBAC
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.8. Production verification record — cost allocation
- **Topic:** Pipeline service accounts
- **Risk addressed:** cross-project access
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.9. Production verification record — cleanup
- **Topic:** Pipeline service accounts
- **Risk addressed:** unsigned images
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.10. Production verification record — legal hold
- **Topic:** Pipeline service accounts
- **Risk addressed:** token overreach
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.11. Production verification record — testing
- **Topic:** Pipeline service accounts
- **Risk addressed:** default service account misuse
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.12. Production verification record — unit test
- **Topic:** Pipeline service accounts
- **Risk addressed:** secret exposure
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.13. Production verification record — integration test
- **Topic:** Pipeline service accounts
- **Risk addressed:** wildcard RBAC
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.14. Production verification record — failure injection
- **Topic:** Pipeline service accounts
- **Risk addressed:** cross-project access
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.15. Production verification record — load test
- **Topic:** Pipeline service accounts
- **Risk addressed:** unsigned images
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.16. Production verification record — GitOps
- **Topic:** Pipeline service accounts
- **Risk addressed:** token overreach
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.17. Production verification record — CI validation
- **Topic:** Pipeline service accounts
- **Risk addressed:** default service account misuse
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.18. Production verification record — policy as code
- **Topic:** Pipeline service accounts
- **Risk addressed:** secret exposure
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.19. Production verification record — documentation
- **Topic:** Pipeline service accounts
- **Risk addressed:** wildcard RBAC
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.20. Production verification record — support model
- **Topic:** Pipeline service accounts
- **Risk addressed:** cross-project access
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.21. Production verification record — architecture boundary
- **Topic:** Pipeline service accounts
- **Risk addressed:** unsigned images
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.22. Production verification record — business objective
- **Topic:** Pipeline service accounts
- **Risk addressed:** token overreach
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.23. Production verification record — data contract
- **Topic:** Pipeline service accounts
- **Risk addressed:** default service account misuse
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.24. Production verification record — input validation
- **Topic:** Pipeline service accounts
- **Risk addressed:** secret exposure
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.25. Production verification record — output validation
- **Topic:** Pipeline service accounts
- **Risk addressed:** wildcard RBAC
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.26. Production verification record — idempotency
- **Topic:** Pipeline service accounts
- **Risk addressed:** cross-project access
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.27. Production verification record — retry safety
- **Topic:** Pipeline service accounts
- **Risk addressed:** unsigned images
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.28. Production verification record — timeout strategy
- **Topic:** Pipeline service accounts
- **Risk addressed:** token overreach
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.29. Production verification record — resource requests
- **Topic:** Pipeline service accounts
- **Risk addressed:** default service account misuse
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.30. Production verification record — resource limits
- **Topic:** Pipeline service accounts
- **Risk addressed:** secret exposure
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.31. Production verification record — CPU sizing
- **Topic:** Pipeline service accounts
- **Risk addressed:** wildcard RBAC
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.32. Production verification record — memory sizing
- **Topic:** Pipeline service accounts
- **Risk addressed:** cross-project access
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.33. Production verification record — GPU scheduling
- **Topic:** Pipeline service accounts
- **Risk addressed:** unsigned images
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.34. Production verification record — node placement
- **Topic:** Pipeline service accounts
- **Risk addressed:** token overreach
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.35. Production verification record — taints and tolerations
- **Topic:** Pipeline service accounts
- **Risk addressed:** default service account misuse
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.36. Production verification record — topology
- **Topic:** Pipeline service accounts
- **Risk addressed:** secret exposure
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.37. Production verification record — service account
- **Topic:** Pipeline service accounts
- **Risk addressed:** wildcard RBAC
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.38. Production verification record — RBAC
- **Topic:** Pipeline service accounts
- **Risk addressed:** cross-project access
...
- **Audit record:** Save timestamp, operator, commit, image digest, pipeline version, run identifier, and ticket.
- **Review:** A second engineer confirms the evidence and closes any gap.
### 20.39. Production verification record — secret handling
- **Topic:** Pipeline service accounts
- **Risk addressed:** unsigned images
